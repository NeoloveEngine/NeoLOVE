...
- Common: `Rect2D`, `Shape2D`, `TextBox`, `TextLabel`, `Sprite2D`, `Image2D`,
  `NineSliceSprite2D`, `TileTexture2D`, `EntityScaler`, `Collider2D`, and
  `Rigidbody2D`.
...
Components can be reordered conceptually by removing and re-adding; each
component header has a copy button, and the **Add Component** menu offers
**Paste** to apply a copied component to another entity. Color properties (and
the scene background) open a picker that toggles between an HSV square with a
hue strip and plain RGBA sliders; the choice is remembered in `editor.json`.
